...
const getCovidInformation = (prediction = '', confidence = 0) => {
  const p = prediction.toLowerCase();

  let type = 'normal';
  if (p.includes('covid')) type = 'covid';
  else if (p.includes('viral') || p.includes('pneumonia')) type = 'viral_pneumonia';
  ...
  const map = {
    covid: {
      ...
      icon: '⚠️',
      severity: 'High Risk',
      description: 'Possible Covid-19 related pattern detected.'
    },
    viral_pneumonia: {
      name: 'Viral Pneumonia Pattern',
      color: 'warning',
      icon: '🫁',
      severity: 'Moderate Risk',
      description: 'Possible viral pneumonia pattern detected.'
    },
    normal: {
      name: 'Normal',
      color: 'success',
      icon: '✅',
      severity: 'Low Risk',
      description: 'No abnormal Covid-19 pattern detected.'
    }
  };
  ...
  return { ...map[type], confidenceLevel };
};

export default function CovidAnalysis({ user, token, apiBaseUrl }) {
  ...
  const handleApiPredict = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`${apiBaseUrl}/api/covid_19/predict`, formData, {
        headers: { ...authHeaders, 'Content-Type': 'multipart/form-data' }
      });

      const confidence = data.confidence_percentage ?? (data.confidence || 0) * 100;
      const covidInfo = getCovidInformation(data.prediction, Number(confidence));
      setApiResponse({ ...data, confidence_percentage: Number(confidence), covidInfo });
    } catch (err) {
  ...
  const handleDebugPredict = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${apiBaseUrl}/api/covid_19/debug/prediction`,
        formData,
  ...
  const handleBatchPredict = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${apiBaseUrl}/api/covid_19/predict/batch`,
        formData,
        { headers: { ...authHeaders, 'Content-Type': 'multipart/form-data' } }
      );
  ...
  const loadHistoryAndAnalytics = async () => {
    ...
    try {
      const [h, a] = await Promise.all([
        axios.get(`${apiBaseUrl}/api/covid_19/history`, { headers: authHeaders }),
        axios.get(`${apiBaseUrl}/api/covid_19/analytics`, { headers: authHeaders })
      ]);
  ...
  const loadSystemInfo = async () => {
    ...
    try {
      const [health, classes, model] = await Promise.all([
        ...
        axios.get(`${apiBaseUrl}/api/covid_19/classes`),
        axios.get(`${apiBaseUrl}/api/covid_19/model/info`)
      ]);
  ...
  const loadCharts = async () => {
    ...
    try {
      const [charts, stats] = await Promise.all([
        axios.get(`${apiBaseUrl}/api/covid_19/results/charts`, { headers: authHeaders }),
        axios.get(`${apiBaseUrl}/api/covid_19/results/statistics`, { headers: authHeaders })
      ]);
  ...
  const downloadSingleReport = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${apiBaseUrl}/api/covid_19/predict/report`,
        payload,
      ...
      a.href = url;
      a.download = `covid_report_${Date.now()}.pdf`;
  ...
  const downloadBatchReport = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${apiBaseUrl}/api/covid_19/batch/report`,
        payload,
        { headers: { ...authHeaders, 'Content-Type': 'application/json' }, responseType: 'blob' }
      );
      const blob = new Blob([response.data], { type: 'application/pdf' });
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `covid_batch_report_${Date.now()}.pdf`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
  ...
  return (
    <div className="container py-4 disease-page">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <div>
          <h3 className="mb-0">🫁 Covid-19 Analysis</h3>
            ...
            <div className="card shadow-sm">
              ...
              <div className="card-body">
                ...
                <div className="d-grid gap-2">
                  <button className="btn btn-info" onClick={handleApiPredict} disabled={loading || !selectedFile}>
          ...
          <div className="col-lg-7">
            {apiResponse && (
              <div className="card shadow-sm mb-3">
                <div className={`card-header bg-${apiResponse.covidInfo.color} text-white`}>
                  {apiResponse.covidInfo.icon} {apiResponse.covidInfo.name}
                </div>
                <div className="card-body">
                  <p className="mb-2">{apiResponse.covidInfo.description}</p>
                  <div className="row">
        ...
        <div className="card shadow-sm">
          ...
          <div className="card-body">
            ...
            <div className="d-flex gap-2 mb-3">
              <button className="btn btn-info" onClick={handleBatchPredict} disabled={batchLoading || !selectedFiles.length}>
